...
-- player has enough.
playCard :: NonInteractive m => Age -> PlayerId -> MS.MultiSet Resource -> Card -> m Funding
playCard age pid extraResources card = do
    ...
    -- Retrieve the money generated by this card at this point. This is
    -- buggy as the funding will be play-order dependent.
    getCardFunding pid card
...
-- (resulting from exchanges, or cards played).
resolveAction :: NonInteractive m => Age -> PlayerId -> ([Card], (PlayerAction, Exchange)) -> m ([Card], AddMap PlayerId Funding)
resolveAction age pid (hand, (PlayerAction actiontype card, exch)) = do
    ...
    extramoney <- case actiontype of
                      Drop -> discardpile %= (card :) >> return 3
                      Play -> playCard age pid extraResources card
                      BuildCompany -> do
    ...
    return (filter (/= card) hand, payout <> AddMap (M.singleton pid extramoney))

-- | Play a turn :
-- * Ask the player what he'd like to do with the proposed hand.
-- * Remove the card the player chose from the hand, and play it.
--
-- Not that all cards effects are revealed simultaneously.
...
playTurn age turn rawcardmap = do
    ...
    let payouts = results ^. traverse . _2
    ifor_ payouts $ \pid payout -> playermap . ix pid . pFunds += payout
    -- Now check for players with the Recycling capability
    -- TODO recycling capability
    pure (fmap fst results)
